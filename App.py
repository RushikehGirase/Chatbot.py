...
try:
    with open('C:/Users/Om/Desktop/Chatbot/intents.json') as file:
        intents_data = json.load(file)
except FileNotFoundError:
    st.error("Error: 'intents.json' file not found. Ensure the file exists at the specified path.")
    exit()
except json.JSONDecodeError:
    st.error("Error: Failed to parse 'intents.json'. Ensure it is a valid JSON file.")
    exit()
...
try:
    ...
    st.error(f"Error: {e.filename} file not found. Ensure all necessary files are in place.")
    exit()
...
# Streamlit app UI
st.title("Movie Search Chatbot")
st.write("Hello! I am your Movie Search Chatbot. Ask me about movies or type 'quit' to stop.")
